...
namespace DotNetNuke.Modules.MyDNN.Components
{
    /// -----------------------------------------------------------------------------
    /// <summary>
    /// The Controller class for MyDNN
    /// 
    ...
    public class FeatureController : ModuleSearchBase, IPortable, IUpgradeable
    {
        ...
        public override IList<SearchDocument> GetModifiedSearchDocuments(ModuleInfo moduleInfo, DateTime beginDate)
        {
            ...
            foreach (var item in items)
            {
                ...
                var content = string.Format("{0}<br />{1}", item.ItemTitle, item.ItemContent);
        ...
        public string ExportModule(int moduleId)
        {
            ...
            foreach (Item item in itemList)
            {
                ...
                sb.AppendFormat("<IconId>{0}</IconId>", item.IconId);
                sb.AppendFormat("<ItemTitle>{0}</ItemTitle>", item.ItemTitle);
                sb.AppendFormat("<CreatedOnDate>{0}</CreatedOnDate>", item.CreatedOnDate);
                sb.AppendFormat("<ItemId>{0}</ItemId>", item.ItemId);
                sb.AppendFormat("<ItemReadMore>{0}</ItemReadMore>", item.ItemReadMore);
                sb.AppendFormat("<ItemContent>{0}</ItemContent>", XmlUtils.XMLEncode(item.ItemContent));
        ...
        public void ImportModule(int moduleId, string content, string version, int userId)
        {
            ...
            foreach (XmlNode item in xmlNodeList)
            {
                ...
                newItem.IconId = int.Parse(item.SelectSingleNode("IconId").InnerText, NumberStyles.Integer);
                newItem.ItemTitle = item.SelectSingleNode("ItemTitle").InnerText;
                newItem.ItemContent = item.SelectSingleNode("ItemContent").InnerText;
                newItem.ItemId = int.Parse(item.SelectSingleNode("ItemId").InnerText, NumberStyles.Integer);
                newItem.ItemReadMore = item.SelectSingleNode("ItemReadMore").InnerText;
